<template>
	...
	>
		<template v-slot:container>
			<div class="fetch-news-panel-container">
				<div class="form-block">
					...
					<fv-text-box
						...
						@keydown.enter="fetchNews"
					/>
				</div>
				<div class="form-block">
					<p class="panel-title">{{ local("Max Pages") }}</p>
					<fv-text-box
		...
		<template v-slot:footer>
			<fv-button
				...
				style="width: 140px; color: #f3e7c2"
				@click="fetchNews"
			>
				{{ local("Fetch News") }}
			</fv-button>
...
export default {
	...
	methods: {
		...
		setTask(data = {}) {
			...
			if (
				(this.form.max_pages === "" || this.form.max_pages == null) &&
				(this.task.params?.max_pages != null ||
					this.task.params?.max_blocks != null)
			) {
				this.form.max_pages = `${
					this.task.params.max_pages ?? this.task.params.max_blocks
				}`;
			}
		},
		async fetchNews() {
			...
			const maxPages =
				this.form.max_pages === "" || this.form.max_pages == null
					? undefined
					: Number(this.form.max_pages);
			if (
				maxPages !== undefined &&
				(!Number.isFinite(maxPages) || maxPages <= 0)
			) {
				this.lock.fetch = true;
				this.$barWarning(this.local("Max Pages must be a positive number"), {
					status: "warning",
				});
				return;
			}
			try {
				const res = await this.$api.News.FetchNews(
					this.form.start_url,
					maxPages,
				);
		...
		async getStatus() {
			...
			try {
				const res = await this.$api.News.GetFetchNewsStatus();
				if (res.code === 200 || res.status === "success") {
					this.setTask(res.data || {});
				} else {
					this.$barWarning(
