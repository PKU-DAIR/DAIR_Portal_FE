		...
		<div class="row main-table">
			...
			>
				...
				<template v-slot:column_4="x">
					<fv-tag
						:theme="theme"
						:model-value="[{ text: x.item.email }]"
					></fv-tag>
				</template>
				<template v-slot:column_5="x">
					<fv-tag
						:theme="theme"
						:model-value="[{ text: x.item.invite_code }]"
					></fv-tag>
				</template>
				<template v-slot:column_6="x">
					<fv-tag
						:theme="theme"
						:model-value="[{ text: x.item.phone }]"
					></fv-tag>
				</template>
				<template v-slot:column_7="x">
					<fv-tag
						:theme="theme"
						:model-value="getRole(x.item.role)"
					></fv-tag>
				</template>
		...
		resetPassword(item) {
			...
			this.$infoBox(
				...
				),
				{
					...
					cancelTitle: "Cancel",
					confirm: () => {
						this.$api.User.ResetUserPassword(null, {
							userid: item.userid,
							pwd: "",
							confirm_pwd: "",
						})
